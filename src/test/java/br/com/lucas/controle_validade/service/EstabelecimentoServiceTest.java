...
class EstabelecimentoServiceTest {
    ...
    void deveAtualizarSomenteNomeDoEstabelecimento() {
        UUID id = UUID.randomUUID();
        User user = new User(UUID.randomUUID(), "Lucas", "a@b.com", "123456",
                LocalDateTime.now(), List.of());
        Estabelecimento estabelecimento = new Estabelecimento(id, "Mercado", "m@e.com", "123",
                "999", "Rua A", user, List.of());
        when(repositoryEstabelecimento.findById(id)).thenReturn(Optional.of(estabelecimento));
        when(repositoryEstabelecimento.save(estabelecimento)).thenReturn(estabelecimento);

        var resultado = service.atualizarEstabelecimento(id,
                new EstabelecimentoUpdateDTO("Mercado Central", null, null, null, null));

        assertEquals("Mercado Central", resultado.nome());
        assertEquals("m@e.com", resultado.email());
        assertEquals("123", resultado.cnpj());
        verify(validacaoNomeEstabelecimentoUnico).validar("Mercado Central");
        verify(repositoryEstabelecimento).save(estabelecimento);
    }
    ...
    void deveAtualizarMultiplosCamposDoEstabelecimentoEPreservarRelacionamento() {
        ...
        var resultado = service.atualizarEstabelecimento(id,
                new EstabelecimentoUpdateDTO(null, "novo@e.com", null, "888", "Rua B"));
    ...
    void deveFalharAoAtualizarEstabelecimentoInexistente() {
        UUID id = UUID.randomUUID();
        when(repositoryEstabelecimento.findById(id)).thenReturn(Optional.empty());

        assertThrows(RecursoNaoEncontradoException.class,
                () -> service.atualizarEstabelecimento(id,
                        new EstabelecimentoUpdateDTO("Novo", null, null, null, null)));
        verify(repositoryEstabelecimento, never()).save(any());
    }
}
